...
class HasPausedMedia {
  BehaviorSubject _counter = BehaviorSubject.seeded(false);

  Observable get stream$ => _counter.stream;
  bool get current => _counter.value;

  toglePausePlay(bool isPlaying) {
    _counter.add(isPlaying);
  }
}
          ...
          switch (snapshot.connectionState) {
            ...
            default:
              return snapshot.data.documents.length > 0
                      ...
                      itemBuilder: (context, position) {
                        ...
                        return Container(
                          ...
                          child: Stack(
                            children: <Widget>[
                              StreamBuilder(
                                builder: (BuildContext context, videosnapshot) {
                                  return AppVideoPlayer(
                                      url:
                                          '${snapshot.data.documents[position]['servicePhoto']}');
                                },
                                stream: HasPausedMedia().stream$,
                              ),
                              onScreenControls(
                                caption:
                                    '${snapshot.data.documents[position]['storyTitle']}',
                                comment:
                                    '${snapshot.data.documents[position]['comment_sample']}',
                                commenterPhotoUrl:
                                    '${snapshot.data.documents[position]['commenterPhotoUrl']}',
                                commentNumbers:
                                    '${snapshot.data.documents[position]['commentNumbers']}',
                                likes:
                                    '${snapshot.data.documents[position]['likes']}',
                                username:
                                    '${snapshot.data.documents[position]['fullName']}',
                                userPhotoUrl:
                                    '${snapshot.data.documents[position]['profilePicture']}',
                                docID:
                                    '${snapshot.data.documents[position]['doc_id']}',
                                uid: uid,
                                shareUrl:
                                    '${snapshot.data.documents[position]['servicePhoto']}',
                                userId:
                                    '${snapshot.data.documents[position]['userId']}',
                                postion: position,
                              )
                            ],
                          ),
                  ...
                  : Center(
                      child: Text(
                        'Looks like no story has been shared.',
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      child: new Scaffold(
          ...
          body: Stack(
            children: <Widget>[_videoStories(context), homeHeader(context)],
          ), // ,homeHeader()
      ...
      onWillPop: () async {
        Navigator.pushReplacement(context,
            MaterialPageRoute(builder: (context) => StylesBeautyWidget()));
...
class _AppVideoPlayerState extends State<AppVideoPlayer> {
  ...
  var state = HasPausedMedia().stream$;
  var isToggled = false;
  @override
  void initState() {
    // print(widget.url);
    ...
    print('Streamed ma gialsssssssssssssssssssss $isToggled');
    state.listen((onData) {
      if (onData == true) {
        print('voxxxx ma gialsssssssssssssssssssss $isToggled');
        setState(() {
          _controller.pause();
        });
      }
  ...
  Widget build(BuildContext context) {
    return Center(
      child: _controller.value.initialized
          ? StreamBuilder(
              builder: (BuildContext context, snapcore) {
                return InkWell(
                  child: AspectRatio(
                  ...
                  onTap: () {
                    setState(() {
                      if (_controller.value.isPlaying) {
                        _controller.pause();
                        HasPausedMedia().toglePausePlay(true);
                        print('my snapcore $snapcore');
                      } else {
                        HasPausedMedia().toglePausePlay(false);
                        print('my snapcore2 $snapcore');
                        _controller.play();
                      }
                    });
                  },
...
class _onScreenControlsState extends State<onScreenControls> {
  ...
  Widget videoControlAction(
      {IconData icon, String label, double size = 26, int index = 0}) {
    return Padding(
        ...
        children: <Widget>[
          ...
          Padding(
            ...
            child: Text(
              label ?? "",
              style: TextStyle(
                  fontSize: 10, color: index == 1 ? Colors.red : Colors.white),
            ),
          )
        ],
      ),
        ...
        builder: (BuildContext context, snapshot) {
          try {
            return videoControlAction(
                ...
                label: '${widget.likes == '0' ? '' : widget.likes}',
    ...
    if (permissionStatus == PermissionStatus.granted) {
      ...
      try {
        ...
        await dio.download(url, dirloc + randid + ".mp4",
          ...
          setState(() {
            shouldTogglePlay.add(index);
            progress = ((receivedBytes / totalBytes) * 100).toInt().toString();
          });
          if (progress == '100') {
            // dirloc +
      ...
      child: Row(
        children: <Widget>[
          Expanded(
              flex: 5,
              child:
                  videoDesc(widget.username, widget.caption, widget.comment)),
          Expanded(
            flex: 1,
            child: Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                  InkWell(
                    ...
                    onTap: () {
                      Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                              builder: (context) => VideoStoriesByUser(
                  ...
                  InkWell(
                    child: controlStream(
                        docID: '${widget.docID}', uid: '${widget.uid}'),
                    onTap: () {
                      try {
                        Firestore.instance
                            .collection('users')
                          ...
                          if (onValue.exists && onValue.data['likes'] == 1) {
                            Firestore.instance
                                ...
                                .collection('likes')
                                .document('${widget.docID}')
                                .setData({'likes': 0}, merge: true).then(
                                    (removeCount) {
                              Firestore.instance
                                  .collection('userServiceVideos')
                                  .document('${widget.docID}')
                                ...
                                Firestore.instance
                                    ...
                                    .document('${widget.docID}')
                                    .setData({
                                  'likes': likesDecrement.data['likes'] - 1
                                }, merge: true).then((isDone) {
                                  print('is done');
                                });
                              });
                            });
                            ...
                            Firestore.instance
                              ...
                              Firestore.instance
                                  .collection('userServiceVideos')
                                  .document('${widget.docID}')
                                  .setData({'likes': likeVal.data['likes'] + 1},
                  ...
                  InkWell(
                    child: videoControlAction(
                        ...
                        label:
                            '${widget.commentNumbers == '0' ? '' : widget.commentNumbers}'),
                    onTap: () {
                  ...
                  InkWell(
                    child: videoControlAction(
                        ...
                        label: '$progress',
                        index:
                            shouldTogglePlay.contains(widget.postion) ? 1 : 0),
                    onTap: () {
                      // tashi
                      downloadFile(widget.shareUrl, widget.postion,
                  ...
                  //     },),
                  widget.commenterPhotoUrl != 'N/A'
                      ? SpinnerAnimation(
                          body: audioSpinner(widget.commenterPhotoUrl))
                      : SizedBox.shrink()
                ],
              ),
            ),
