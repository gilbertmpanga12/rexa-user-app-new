...
class OrderPageState extends State<OrderPage> {
  ...
  _launchURL(String website, bool shouldToggle, String bookingUrl,
  ...
  shipUrl(String website, bool shouldToggle, String bookingUrl,
      String shippingUrl) async {
    Navigator.push(
        context,
        MaterialPageRoute(
            ...
            child: StreamBuilder(
              builder: (context, snapshot) {
                ...
                return snapshot.data.documents.length > 0
                                  ...
                                  ListTile(
                                    ...
                                    leading: CircleAvatar(
                                      ...
                                      backgroundImage: NetworkImage(
                                          '${snapshot.data.documents[index]['photoUrl'] ?? defaultPicture}'),
                                    ),
                                    title: Column(
                                      mainAxisAlignment:
                    ...
                    : Center(
                        child: Text('No reviews for this service provider',
                            style: TextStyle(
                              color: Colors.white,
                            )),
                      );
              ...
              stream: Firestore.instance
                  ...
                  .document(uid)
                  .collection('reviews')
                  .where('comment_length', isGreaterThan: 1)
                  .snapshots(),
            ),
          );
        });
                ...
                onPressed: () async {
                  try {
                    ...
                    Firestore.instance
                        .collection('orders')
                        .document(
                            '${prefs.getString('lastOrderedServiceProviderId')}')
                        .delete()
                        .then((onvalue) {
                      Firestore.instance
                          .collection('saloonServiceProvider')
                          .document('${_uid}')
                          .setData(
                              {'isRequested': false, 'status_notifier': false},
                              merge: true).then((newval) {
                        ...
                        Firestore.instance
                            .collection('users')
                            .document('${_userId}')
                            .updateData({'request_made': false}).then((val) {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
  ...
  fetchCategories() async {
    try {
      final response = await http.get(
          'https://yiiyaa-app.herokuapp.com/api/get-service-provider/$_uid');
...
* */
  ...
  requestServiceNotifier(
      String playerId, String contents, String headings) async {
  ...
  makeServiceRequest() async {
    ...
    final response = await http.post(
        'https://yiiyaa-app.herokuapp.com/api/make-request',
        body: _payload,
        headers: {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      Firestore.instance
          .collection('users')
          .document('$_userId')
          .updateData({'request_made': true});
      requestServiceNotifier(_fcmToken, "New Service Request",
...
//                                    color: Colors.green,
                        ...
                        ListTile(
                            ...
                            subtitle: Padding(
                              child: Text('${_requestedSaloonService}',
                                  style: TextStyle(fontSize: 17.0)),
                        ...
                        ListTile(
                            ...
                            subtitle: Padding(
                                child: Text('${_requestedDescription}',
                                    style: TextStyle(
                                      fontSize: 17.0,
                                    )),
                                padding:
                                    EdgeInsets.only(top: 3.0, bottom: 4.0))),
                        Container(
                          child: Divider(
                            indent: 57.8,
                          ),
          ...
          bottomNavigationBar: Row(
            ...
            children: <Widget>[
              RaisedButton(
                ...
                onPressed: () {
                  if (_website == 'null') {
                    Toast.show('Oops website url not available', context,
                ...
                child: RaisedButton(
                  ...
                  onPressed: () {
                    if (_shippingAddress == 'null') {
                      Toast.show(
                          'Oops shipping address  not available', context,
                          backgroundColor: Colors.red, duration: 5);
                    } else {
                      shipUrl('$_shippingAddress', false, _website,
                          _shippingAddress);
                    }
                  },
                  color: Colors.white,
