...
class UserProfileState extends State<UserProfile> {
    ...
    ImagePicker.pickImage(source: ImageSource.gallery).then((image) {
      ...
      task.onComplete.then((image) {
        firebaseStorageRef.getDownloadURL().then((result) {
          ...
          http.post(
              'https://yiiyaa-app.herokuapp.com/api/update-profile-picture',
              ...
              showDialog(
                  ...
                  builder: (BuildContext context) {
                    return AlertDialog(
                      ...
                      content: Text('Please try again or contact support team'),
                      actions: <Widget>[
                        FlatButton(
                          child: Text('OK'),
                          onPressed: () {
  ...
  void _submitUsername() async {
    Firestore.instance
        ...
        .updateData({'fullName': controller.value.text}).then((resp) {
      controller.clear();
  ...
  void _submitAbout() async {
    Firestore.instance
        .collection('users')
        .document('${_firebaseUID}')
        .updateData({'about': controller.value.text}).then((resp) {
      controller.clear();
      Navigator.pop(context);
    });
  }
  ...
  void _settingModalBottomSheet(
      context, String actionPlaceholder, String fullName) {
                              ...
                              children: <Widget>[
                                ...
                                ListTile(
                                  onTap: () {
                                    ...
                                    _settingModalBottomSheet(
                                        context,
                                        'Add About',
                                        '${snapshot.data['about'] == null ? 'Not Available' : snapshot.data['about']}');
                                  },
                                  trailing: Icon(Icons.edit),
                                ...
                                ListTile(
                                  leading: Icon(Icons.phone,
                                      color: Colors.yellow[800]),
                                  title: Text(
                                      DemoLocalizations.of(context).telephone,
                                      style: TextStyle(
                                          fontWeight: FontWeight.w500,
                                          fontSize: 14)),
